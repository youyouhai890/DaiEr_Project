...
#pragma target 3.0
	...
	struct Input {
		...
		float3 worldRefl;		//面编程采样时用到的,向量位置
	};
	...
	UNITY_INSTANCING_BUFFER_START(Props)
		...
		float3 SheXiangJiDaoDingDian = -WorldSpaceViewDir(v.vertex);

		//把法向量变换到世界空间,跟视向量统一坐标空间,
		float3 N = mul(v.normal, (float3x3)unity_WorldToObject);
		N = normalize(N);
		//获取反射向量R
		o.worldRefl = reflect(SheXiangJiDaoDingDian, N);
	...
	void surf(Input IN, inout SurfaceOutputStandard o) {
		...
		float4 col = texCUBE(_CubeMap, IN.worldRefl);
